...
│         │  scrape                                                 │
│         ▼                                                         │
│   unbound-exporter / kea-exporter (cross-namespace)               │
│                                                                   │
│  MetalLB ─── LoadBalancer IP for DNS / DHCP / Grafana             │
└──────────────────────────────────────────────────────────────────┘
```
...
| コンポーネント | イメージ | 役割 |
|---|---|---|
| Unbound | `klutchell/unbound:latest` | DNS キャッシュリゾルバ (cachedb + Redis) |
...
│       ├── servicemonitors/    # Unbound / Kea DHCP ServiceMonitor
...
`base/unbound/configs/forward-zones.conf` で上位 DNS サーバーを指定:
...
#### 計測結果 (2026-02-24, OrbStack K8s single node)
...
| Pod IP 直接負荷分散 (クライアント側) | 検証用 | svclb ボトルネックの裏取り |
...
Unbound / Kea DHCP のサイドカー Exporter から ServiceMonitor 経由で cross-namespace scrape:
...
- `klutchell/unbound` はミニマルイメージ (`sh`, `cat`, `ls` 等なし)
